...
class Gerstlix {
  ...
  async getInfo(server) {
    if (server && acceptServers.includes(server)) {
      ...
      return { success: false, data: `Сервер: ${server} не находится в одобренном списке` };
    }
  }
  ...
  async getMembers(server, fractionId) {
    if (server && fractionId && acceptServersArz) {
      ...
      return { success: false, data: `Проверьте serverID и fractionID на валидность` };
    }
  }
  ...
  async getLeadersList(server) {
    if (server && acceptServers.includes(server)) {
      ...
      return { success: false, data: `[getLeadersList] Сервер: ${server} не находится в одобренном списке` };
    }
  }
  ...
  async getMinister(server, fractionId) {
    if (server && fractionId && acceptServers.includes(server)) {
      ...
      return { success: false, data: "[getMinister] Проверьте serverID и fractionID на валидность" };
    }
  }
  ...
  async getMinistersList(server) {
    if (server && acceptServers.includes(server)) {
      ...
      return { success: false, data: "[getMinistersList] Укажите валидные параметры!" };
    }
  }
  ...
  async getRecordFraction(server, fractionId) {
    if (server && fractionId && acceptServers.includes(server)) {
      ...
      return { success: false, data: "[getRecordFraction] Укажите валидные параметры!" };
    }
  }
  ...
  async getRecords(server) {
    if (acceptServers.includes(server)) {
      ...
      return { success: false, data: `Сервер ${server} не находится в списке одобренных:\n${acceptServers}` };
    }
  }
  ...
  async getStatus(gameProject) {
    if (acceptProject.includes(gameProject)) {
      ...
      return { success: false, data: `Укажите один из проектов: ${acceptProject}` };
    }
  }
  ...
  async getPlayer(server, player) {
    if (server && player) {
      if (typeof server === "number" && typeof player === "string") {
        ...
        console.log(`Укажите верный тип данных`);
      }
    } else {
      return { success: false, data: `Укажите параметры к методу` };
    }
  }
  ...
  async geoIp(ip) {
    if (ip) {
      ...
      return { success: false, data: "Укажите IP!" };
    }
  }
}
